...
public class SolarSystemBuilder : MonoBehaviour
{
    public static void BuildCompleteSolarSystem()
    {
        ...
        sunObj.transform.localScale = Vector3.one * 16f;
        ...
        sunBody.bodyRadius = 8f;
        sunBody.rotationSpeed = 2f;
        ...
        sunLight.intensity = 150f;
        sunLight.range = 1000f;
        sunLight.shadows = LightShadows.Soft;

        manager.centralStar = sunBody;

        // 5. Create Planets and Moons
        // (Name, OrbitRadius, OrbitSpeed, Size, Color, RotationSpeed, AxialTilt, MatKey, Description, Facts)
        
        // --- MERCURY ---
        CelestialBody mercury = CreatePlanet(root.transform, sunObj.transform, "Mercure", 28f, 18f, 1.2f, mats["Mercury"], 6f, 0.03f,
        ...
        // --- VENUS ---
        CelestialBody venus = CreatePlanet(root.transform, sunObj.transform, "Vénus", 42f, 13f, 2.2f, mats["Venus"], -3f, 177.3f,
        ...
        // --- EARTH & MOON ---
        CelestialBody earth = CreatePlanet(root.transform, sunObj.transform, "Terre", 60f, 10f, 2.5f, mats["Earth"], 15f, 23.4f,
        ...
        CelestialBody moon = CreateMoon(root.transform, earth.transform, "Lune", 6.5f, 40f, 0.7f, mats["Moon"], 10f,
        ...
        // --- MARS & MOONS ---
        CelestialBody mars = CreatePlanet(root.transform, sunObj.transform, "Mars", 80f, 8f, 1.6f, mats["Mars"], 14f, 25.2f,
        ...
        CelestialBody phobos = CreateMoon(root.transform, mars.transform, "Phobos", 3.2f, 65f, 0.35f, mats["Phobos"], 20f,
        ...
        CelestialBody deimos = CreateMoon(root.transform, mars.transform, "Déimos", 5.2f, 45f, 0.25f, mats["Deimos"], 15f,
        ...
        // --- ASTEROID BELT ---
        CreateAsteroidBelt(root.transform, 95f, 115f, 250, mats["Asteroid"]);

        // --- JUPITER & MOONS ---
        CelestialBody jupiter = CreatePlanet(root.transform, sunObj.transform, "Jupiter", 140f, 5f, 5.5f, mats["Jupiter"], 25f, 3.1f,
        ...
        CelestialBody io = CreateMoon(root.transform, jupiter.transform, "Io", 9.5f, 50f, 0.55f, mats["Io"], 20f,
        ...
        CelestialBody europa = CreateMoon(root.transform, jupiter.transform, "Europe", 13f, 38f, 0.5f, mats["Europa"], 15f,
        ...
        CelestialBody ganymede = CreateMoon(root.transform, jupiter.transform, "Ganymède", 17f, 28f, 0.8f, mats["Ganymede"], 12f,
        ...
        CelestialBody callisto = CreateMoon(root.transform, jupiter.transform, "Callisto", 21f, 20f, 0.75f, mats["Callisto"], 10f,
        ...
        // --- SATURN & RINGS & MOONS ---
        CelestialBody saturn = CreatePlanet(root.transform, sunObj.transform, "Saturne", 190f, 3.8f, 4.8f, mats["Saturn"], 22f, 26.7f,
        ...
        CreateSaturnRings(saturn.transform, 1.25f, 2.35f, mats["SaturnRings"]);

        CelestialBody titan = CreateMoon(root.transform, saturn.transform, "Titan", 15f, 32f, 0.75f, mats["Titan"], 14f,
        ...
        CelestialBody enceladus = CreateMoon(root.transform, saturn.transform, "Encelade", 9f, 48f, 0.4f, mats["Enceladus"], 20f,
        ...
        // --- URANUS & MOONS ---
        CelestialBody uranus = CreatePlanet(root.transform, sunObj.transform, "Uranus", 240f, 2.7f, 3.5f, mats["Uranus"], 16f, 97.8f,
        ...
        CelestialBody titania = CreateMoon(root.transform, uranus.transform, "Titania", 8f, 35f, 0.45f, mats["Titania"], 18f,
        ...
        CelestialBody oberon = CreateMoon(root.transform, uranus.transform, "Obéron", 12f, 25f, 0.42f, mats["Oberon"], 15f,
        ...
        // --- NEPTUNE & TRITON ---
        CelestialBody neptune = CreatePlanet(root.transform, sunObj.transform, "Neptune", 290f, 2.1f, 3.4f, mats["Neptune"], 15f, 28.3f,
        ...
        CelestialBody triton = CreateMoon(root.transform, neptune.transform, "Triton", 9f, -32f, 0.55f, mats["Triton"], 16f,
        ...
        // --- PLUTO & CHARON ---
        CelestialBody pluto = CreatePlanet(root.transform, sunObj.transform, "Pluton", 335f, 1.6f, 1.1f, mats["Pluto"], 10f, 122.5f,
        ...
        CelestialBody charon = CreateMoon(root.transform, pluto.transform, "Charon", 3.8f, 30f, 0.5f, mats["Charon"], 10f,
        ...
        GameObject shipObj = CreatePlayerSpaceship(root.transform, earth.transform.position + new Vector3(8f, 3f, 8f));
    ...
    private static void CreateAsteroidBelt(Transform root, float innerRadius, float outerRadius, int count, Material asteroidMat)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            float height = Random.Range(-4f, 4f);
    ...
    private static void CreateStarfield(Transform root)
    {
        ...
        ParticleSystem.Particle[] particles = new ParticleSystem.Particle[1200];
        for (int i = 0; i < 1200; i++)
        {
            Vector3 dir = Random.onUnitSphere;
            float dist = Random.Range(800f, 1400f);
            particles[i].position = dir * dist;
            particles[i].startSize = Random.Range(2f, 6f);
            
            float hue = Random.Range(0.55f, 0.65f); // Cool white/blue and faint golden stars
            if (Random.value < 0.2f) hue = Random.Range(0.08f, 0.15f);
            Color starCol = Color.HSVToRGB(hue, Random.Range(0.1f, 0.4f), Random.Range(0.7f, 1f));
            particles[i].startColor = starCol;
        }

        ps.SetParticles(particles, 1200);
    }
